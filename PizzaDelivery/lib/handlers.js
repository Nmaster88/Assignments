...
handlers._users.post = function(data,callback){
  ...
  console.log("name: " + name +", address: "+address+", email: "+email+", password: "+password+", tosAgreement: "+tosAgreement)
...
handlers._users.get = function(data,callback){
  ...
  var email = typeof(data.queryStringObject.email) == 'string' && data.queryStringObject.email.trim().length > 0 && helpers.validateEmail(data.queryStringObject.email) ? data.queryStringObject.email.trim() : false;
  console.log("email: "+ data.queryStringObject.email)
  if(email){
...
// Required data: email
// Only let authenticated users delete their object.
// Cleanup any other data associated with the user
                    ...
                    _data.delete('checks',orderId,function(err){
                      ...
                      if(ordersDeleted == ordersToDelete){
                        if(!deletionErrors){
                          ...
                          callback(500,{'Error' : "Errors encountered while attempting to delete all of the user's orders. All checks may not have been deleted from the system successfully."})
                        }
                      }
                    });
                  });
